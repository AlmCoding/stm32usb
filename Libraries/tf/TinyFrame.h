 ...
 *
 * (c) Ondrej Hruska 2017-2018, MIT License
 ...
 * @param count - num of bytes in the buffer
 */
 ...
 * @param timeout - listener expiration time in ticks
 * @return success
 */
 ...
 */
bool TF_QuerySimple(TinyFrame* tf, TF_TYPE type, const uint8_t* data, TF_LEN len, TF_Listener listener,
                    TF_TICKS timeout);
...
struct TF_IdListener_ {
  TF_ID id;
  TF_Listener fn;
  TF_TICKS timeout;      // num of ticks remaining to disable this listener
...
struct TinyFrame_ {
  ...
  /* Own state */
  TF_Peer peer_bit;  //!< Own peer bit (unique to avoid msg ID clash)
